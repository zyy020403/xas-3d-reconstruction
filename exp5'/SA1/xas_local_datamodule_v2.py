...
xas_local_datamodule_v2.py — Exp4 Step 3 Phase 5b
                              Exp5' STEP1 patch (2026-05-01): inject 5 shell_boundaries
                              training fields into PyG Data wrapper. See
                              EXP5_PRIME_STEP1_HANDOFF.md §3.
...
def _dict_to_pyg_data(s: dict) -> Data:
    ...
    Field mapping:
      ...
      Exp5' STEP1 — shell training inject (graph-level (1,)-tensors → (B,) at batch):
        s["true_shell1_d_mean"]  scalar f32  →  (1,) float32   → batched (B,)
        s["true_shell2_d_mean"]  scalar f32  →  (1,) float32   → batched (B,)
        s["has_shell2"]          scalar bool →  (1,) bool      → batched (B,)
        s["true_shell1_n"]       scalar long →  (1,) long      → batched (B,)
        s["true_shell2_n"]       scalar long →  (1,) long      → batched (B,)
        Consumed by _shell_distance_loss / _shell_count_loss in
        diffusion_w_type_xas.py (forthcoming STEP1 §1.3).
    ...
    data = Data(
        ...
        # ---- Exp5' STEP1: 5 shell training fields ----
        # Each input is a 0-d scalar tensor from dataset; .unsqueeze(0) → (1,);
        # Batch.from_data_list stacks along dim=0 → (B,). Consumed by
        # diffusion_w_type_xas.py::_shell_distance_loss / _shell_count_loss.
        true_shell1_d_mean   = s["true_shell1_d_mean"].unsqueeze(0),  # (1,) float32
        true_shell2_d_mean   = s["true_shell2_d_mean"].unsqueeze(0),  # (1,) float32
        has_shell2           = s["has_shell2"].unsqueeze(0),          # (1,) bool
        true_shell1_n        = s["true_shell1_n"].unsqueeze(0),       # (1,) long
        true_shell2_n        = s["true_shell2_n"].unsqueeze(0),       # (1,) long
    )
...
if __name__ == "__main__":
    ...
    if batch is None:
        ...
        print()
        print("=== Exp5' STEP1 inject verify ===")
        for fld, expected_dtype in [
            ("true_shell1_d_mean", torch.float32),
            ("true_shell2_d_mean", torch.float32),
            ("has_shell2",         torch.bool),
            ("true_shell1_n",      torch.long),
            ("true_shell2_n",      torch.long),
        ]:
            v = getattr(batch, fld)
            ok = (v.shape == (4,)) and (v.dtype == expected_dtype)
            print(f"  {fld}: shape={tuple(v.shape)} dtype={v.dtype} -> {'OK' if ok else 'FAIL'}")

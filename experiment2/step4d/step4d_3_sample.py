# step4d_3_sample.py
# Step4d — val/test 集采样（L=6 版本）
# ============================================================
# Step4c agent 警告（坑 4）：
#   原版 step4_3_sample.py 有 pred_frac_centered = pred_frac_all - 0.5
#   Step4c diffusion (v3) sample() 已输出 [-0.5, 0.5]，该行造成双重偏移
#   本脚本已删除该行。
# ============================================================
...
if __name__ == "__main__":
    ...
    datamodule = XASDataModule(
        batch_size=BATCH_SIZE_SAMPLE, num_workers=0, L=L)  # ★ L=6
    ...
    def run_sampling(loader, split_name):
        ...
        for batch_idx, batch in enumerate(tqdm(loader, desc=f"Sampling {split_name}")):
            ...
            # ★ 注意：Step4c diffusion v3 sample() 已输出 [-0.5, 0.5]
            # ★ 不做 -0.5 偏移（原版 step4_3_sample.py 的 pred_frac_centered
            #    = pred_frac_all - 0.5 是旧坐标系补丁，在 v3 下造成双重偏移，已删除）
        ...
        out_path = os.path.join(STEP4d_DIR, f"predictions_{split_name}.pt")
    ...
    logger.info("")
    logger.info("=" * 60)
    logger.info("Step4d 采样完成")
    logger.info(f"  val  → {val_out}")
    logger.info(f"  test → {test_out}")
    logger.info("下一步：运行 step4d_4_compute_metrics.py")
    logger.info("=" * 60)

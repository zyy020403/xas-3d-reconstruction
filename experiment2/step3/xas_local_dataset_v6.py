"""
Step 3.1 — XASLocalStructureDataset  (v6)
==========================================
Dataset for XAS -> local Fe structure prediction (Experiment 2).

修改记录（v6）：
  - [BUG FIX] frac_coords 坐标系统一：
      v5：frac_coords = (neighbor_carts / self.L).copy()
          → 范围 [-0.5, 0.5]（中心化坐标）
      v6：frac_coords = ((neighbor_carts / self.L) % 1.0).copy()
          → 范围 [0, 1]（与 diffusion_w_type_xas.py forward() 中 % 1. 加噪保持一致）
    此修复解决 Step4 中 RMSD ≈ 随机基线（4.51 Å）的根本原因：
    两端坐标系不一致导致原点附近原子坐标被撕裂成双峰分布。

  - 对应地，过滤条件从 abs > 0.5 改为检查是否全在 [0, 1]（通常始终满足）
...
class XASLocalStructureDataset(Dataset):
    ...
    def _load_local_structure(self, folder: str, site_nn: str):
        ...
        # ★ v6 修复：统一坐标系为 [0, 1]，与 forward() 中 % 1. 加噪保持一致
        # v5 错误：frac_coords = (neighbor_carts / self.L).copy()  → [-0.5, 0.5]
        # v6 修正：加 % 1.0，将负值折叠到 [0, 1] 范围
    ...
    def __getitem__(self, index: int):
        ...
        # v6 坐标范围检查：修复后 frac_coords 应全在 [0, 1]
        # （极端情况：neighbor_carts 超出 [-L, L] 范围的样本，% 1.0 后仍在 [0,1]，无需额外过滤）
        if np.any(np.isnan(frac_coords)) or np.any(np.isinf(frac_coords)):
            return None
        ...
        data = Data(
            ...
            lengths       = torch.tensor([L, L, L],   dtype=torch.float32).view(1, -1),
    ...
    def __repr__(self) -> str:
        return f"XASLocalStructureDataset(v6)(n_samples={len(self.samples)}, L={self.L}A)"
...
if __name__ == '__main__':
    ...
    # ── v6 强制验证：frac_coords 必须全部在 [0, 1] ──
    print("\n[v6 验证] 检查 5 个样本的 frac_coords 范围...")
    checked = 0
    for i in range(min(50, len(ds))):
        s = ds[i]
        if s is None:
            continue
        fc = s.frac_coords.numpy()
        fc_min = fc.min()
        fc_max = fc.max()
        print(f"  样本[{i}] mp_id={s.mp_id}  "
              f"frac_coords min={fc_min:.4f}  max={fc_max:.4f}  "
              f"{'✅ [0,1]' if fc_min >= 0 and fc_max <= 1 else '❌ 超出范围！'}")
        checked += 1
        if checked >= 5:
            break

    # ── feff 命中率检查（继承自 v4/v5）──
    print()
    hit = miss = 0
    for i in range(min(50, len(ds))):
        s = ds[i]

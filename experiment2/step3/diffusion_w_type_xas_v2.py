"""
diffusion_w_type_xas.py  (v2 — + density regularization)
=========================================================
Changes from v1:
  1. forward(): add loss_density via Tweedie x0 estimate + minimum-image L2
  2. YAML requires: cost_density (float, e.g. 0.5)
  3. sample():  apply minimum-image fold after denoising (no % 1.)
               so predicted frac_coords are in [-0.5, 0.5] → matches true_frac
"""
...
class CSPDiffusion(BaseModule):
    ...
    def _density_loss(input_frac_coords, pred_x, sigmas_per_atom, sigmas_norm_per_atom):
        ...
        Tweedie (近似):
          tar_x = -rand_x / (sigma * sqrt(sigma_norm))
          pred_x ≈ tar_x
          rand_x_hat = -pred_x * sigma * sqrt(sigma_norm)
          x0_hat = x_t - sigma * rand_x_hat
                 = input_frac_coords + sigma^2 * pred_x * sqrt(sigma_norm)
        """
        with torch.no_grad():
            # 不参与二次梯度，只作为权重
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # sample（推断）★ 关键改动：最终输出不做 % 1.，直接返回 [-0.5, 0.5] 附近的坐标
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def sample(self, batch, diff_ratio=1.0, step_lr=1e-5):
        ...
        # ★ 最终输出：应用最小镜像，使坐标在 [-0.5, 0.5] 与 true_frac 对齐
        final_frac = traj[0]['frac_coords']                # [0, 1]
        final_frac_mi = final_frac.clone()
        final_frac_mi[final_frac_mi > 0.5] -= 1.0         # [-0.5, 0.5]
        ...
        # traj[0] 的 frac_coords 替换为最小镜像版本

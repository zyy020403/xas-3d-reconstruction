# =============================================================================
# 脚本编号: step2.1
# 脚本名称: step2.1_spectrum_encoder.py
# 输入:
#   - experiment/step1/data_inventory.csv  (用于验证测试)
#   - 各位点文件夹下的 chi.dat 文件
# 输出:
#   - 本文件作为模块被 step2.2 和 step2.3 import
#   - 提供: preprocess_chi(), SpectrumEncoder
# 说明:
#   定义 chi.dat 预处理函数和单谱编码器。
#   编码器结构: 1D CNN + 元素 embedding + ionic flag embedding → site embedding
#   d_site = 256（固定），输出将在 Step 2.2 聚合为 d_struct = 256
#   不含任何训练循环，仅模块定义 + __main__ 快速验证。
# =============================================================================
...
if __name__ == "__main__":
    ...
    print(f"  dtype          : {spec.dtype}")          # 应为 torch.float32
    ...
    assert spec3.dtype == torch.float32, "dtype 不是 float32！"
    ...
    if os.path.isfile(inventory_path):
        ...
        if len(valid) > 0:
            ...
            print(f"  dtype          : {spec_real.dtype}")   # 应为 torch.float32
            assert spec_real.dtype == torch.float32, "真实数据 dtype 不是 float32！"
            with torch.no_grad():
